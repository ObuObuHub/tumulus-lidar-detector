...
# TEST CLEAN-SUBSAMPLE: aleg cei mai CURAȚI 3000 DK (scor mic de contaminare șanț) în loc de random
dkscore={r['file']:float(r['score']) for r in csv.DictReader(open('/tmp/dk_scores.csv'))}
postr_dk.sort(key=lambda p:dkscore.get(os.path.basename(p[0]),0.0)); dk_sel=postr_dk[:N_BAL]
print(f"  CLEAN-SUBSAMPLE: DK ales după scor curat (cel mai mare scor în set: {dkscore.get(os.path.basename(dk_sel[-1][0]),0):.2f})",flush=True)
...
torch.save(net2.state_dict(),f'{H}/combined_clean.pt');print("model -> combined_clean.pt")
