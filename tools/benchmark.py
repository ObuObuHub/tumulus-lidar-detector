...
CACHE=os.environ.get("LAKI3_CACHE","/tmp/laki3");CS=0.5;os.makedirs(CACHE,exist_ok=True)
import pyproj
_TF={}
def _tf(s,t):
...
rows=list(csv.DictReader(open(GT,encoding='utf-8-sig')));gll=[(float(r['lon']),float(r['lat'])) for r in rows]  # B3a: utf-8-sig -> GT salvat din Excel ("CSV UTF-8", cu BOM) nu mai crapa cu KeyError('lon')
...
# B3d: refuz ZGOMOTOS daca GT-ul e imprastiat geografic -> mozaicul float32 pe TOT bbox-ul ar da OOM tacut
...
step=int(12/CS)
SCALES=[float(x) for x in os.environ.get('SCALES','80').split(',')]  # default '80' = single-scale production (matches all published figures); SCALES=32,48,68,80 = optional multi-scale survey mode (see EVALUATION 5a). Verified: SCALES=80 reproduces the legacy scorer exactly.
gpos=[(px,py) for py in range(hw,Hh-hw,step) for px in range(hw,W-hw,step)]
scmax=np.full(len(gpos),-1.,np.float32)
def _flush(batch,idxs):
    # scorare pe felii (anti-OOM): nu tine toate stampele in RAM simultan
    if not batch: return
    X=torch.tensor(np.array(batch,dtype=np.uint8))
    with torch.no_grad():
        for k in range(0,len(X),512):
            v=torch.sigmoid(net(X[k:k+512].unsqueeze(1).float().to(dev)/255.)).cpu().numpy()
            for j in range(len(v)):
                gi=idxs[k+j]
                if v[j]>scmax[gi]:scmax[gi]=v[j]
...
for S in SCALES:
    ...
    for gi,(px,py) in enumerate(gpos):
        ...
        w=np.nan_to_num(w,nan=np.nanmedian(w))  # B3c: umple NaN INAINTE de hillshade (ca greens_recall) - altfel percentilele->NaN, guard-ul trece, si o stampa garbage all-255 e scorata tacut
        d2=downs(w,f);h=hs(d2,CS*f);lo,hi=np.percentile(h,2),np.percentile(h,98)
        if not np.isfinite(hi-lo) or hi-lo<1e-6: continue
        ...
        if len(batch)>=4096:_flush(batch,idxs)
    _flush(batch,idxs)
keepmask=scmax>=0
pos=[gpos[i] for i in range(len(gpos)) if keepmask[i]]
sc=scmax[keepmask]
# NMS -> detecții (peak-uri)
NMS=80.0  # B3b: raza NMS FIXA 80m (ca productia), decuplata de MATCH_M care ramane DOAR raza de potrivire la GT
order=np.argsort(-sc);kept=[]
for k in order:
    if sc[k]<0.30: break   # colectez până la 0.30 pt curba PR
    px,py=pos[k]
    if any((px-q[0])**2+(py-q[1])**2<(NMS/CS)**2 for q in kept): continue
...
# operating points
for thr in (0.5,0.6,0.7,0.9):  # A5: include 0.6 = punctul de operare de productie (si 0.5 pe care scriptul deja il printeaza)
