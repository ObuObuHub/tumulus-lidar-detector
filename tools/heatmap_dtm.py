#!/usr/bin/env python3
# heatmap_dtm.py CLON CLAT KM [MODEL] [STEP_M] — HEATMAP OARBĂ pe LAKI3 (Oltenia, 0.5m elevație).
# Versiunea laki3 a heatmap.py (care e doar MDH/vest). Scor dens pe TOATĂ zona via pipeline-ul DTM
# (mozaic elevație -> hillshade 6-dir@2m -> stampă 128 homog), FĂRĂ seeding la ground-truth.
# -> review/heatmap_dtm.png + stat (medie/mediană/%>=0.7/%>=0.9). Necesită tile-uri în /tmp/laki3.
...
if ntiles==0: print(f"EROARE: niciun tile laki3 cache pt {CLON},{CLAT} (caut nk {n0}-{n1}, ek {e0}-{e1} în {CACHE})");sys.exit(1)
print(f"mozaic {W}x{Hh}px ({KM}km, 0.5m), {ntiles} tile-uri, model {os.path.basename(MODEL)}",flush=True)
...
# background hillshade pt display
...
field=np.asarray(Image.fromarray((g*255).astype('uint8')).resize((W,Hh),Image.BICUBIC),np.float32)/255.
s4=field*4;Rr=np.clip(s4-2,0,1);Gg=np.clip(np.minimum(s4,4-s4),0,1);Bb=np.clip(2-s4,0,1)
cmap=np.stack([Rr,Gg,Bb],-1)*255;alpha=np.clip((field-0.3)/0.5,0,1)[...,None]*0.78
base=np.stack([bg,bg,bg],-1).astype(np.float32)
out=(base*(1-alpha)+cmap*alpha).astype('uint8');ov=Image.fromarray(out)
...
dr.text((6,4),f"HEATMAP OARBA DTM {os.path.basename(MODEL)} | medie {valid.mean():.2f} %>=.9 {(valid>=0.9).mean()*100:.0f}% | albastru=mic, rosu=aprinde",fill=(255,255,255),font=ft)
mx=max(W,Hh);disp=ov.resize((int(W*min(1,1200/mx)),int(Hh*min(1,1200/mx))))
out_path=f"{H}/review/heatmap_dtm.png";disp.save(out_path);print(f"  -> {out_path}")
